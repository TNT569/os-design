...
static void drawDiskUsage(void) {
  ...
  printf("Legend: [" COLOR_GREEN "#" COLOR_RESET "]=Used [" COLOR_YELLOW
         "." COLOR_RESET "]=Free\n");
  ...
  int charsPerLine = 64;
  int blocksPerChar = 4;
  for (int line = 0; line < BLOCK_NUM / (charsPerLine * blocksPerChar);
       line++) {
    for (int c = 0; c < charsPerLine; c++) {
      int startBlock = line * charsPerLine * blocksPerChar + c * blocksPerChar;
      int usedCount = 0;
  ...
  for (int i = 0; i < MAX_DIR_ENTRIES; i++) {
    ...
    if (entry.type == TYPE_DIR && entry.subDirBlocks > 0) {
      ...
      for (int j = 0; j < maxEntries; j++) {
        ...
        int subBlockNo = entry.subDirStart + (j * DIR_ENTRY_SIZE) / BLOCK_SIZE;
        int subOffset = (j * DIR_ENTRY_SIZE) % BLOCK_SIZE;
        if (readDiskBlock(subBlockNo, subBlock) != 0) {
          continue;
        }
        memcpy(&subEntry, &subBlock[subOffset], sizeof(DirEntry));
        if (subEntry.name[0] == '\0') {
          continue;
        }
        const char *subType = (subEntry.type == TYPE_DIR) ? "DIR" : "FILE";
        printf("  %-26s %-6s %-8d %-6d\n", subEntry.name, subType,
               subEntry.size, subEntry.blockCount);
      }
    }
  }
  printf("\n");
...
static void drawBufferState(void) {
  ...
  for (int i = 0; i < BUFFER_PAGES; i++) {
    ...
    printf("  %s   ", bufferPool[i].dirty ? COLOR_RED "Y" COLOR_RESET : "N");
    /* 显示前8字节 */
