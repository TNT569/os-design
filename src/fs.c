...
DirEntry *findEntry(const char *path) {
  ...
  if (part2 == NULL) {
    ...
    for (int i = 0; i < MAX_DIR_ENTRIES; i++) {
      ...
      if (entry.name[0] != '\0' && strcmp(entry.name, part1) == 0) {
        result = entry;
        return &result;
      }
    }
  ...
  for (int i = 0; i < maxEntries; i++) {
    if (readSubDirEntry(parent->subDirStart, parent->subDirBlocks, i, &entry) !=
        0) {
      return NULL;
    }
    if (entry.name[0] != '\0' && strcmp(entry.name, part2) == 0) {
      result = entry;
      return &result;
    }
  }
...
int createFile(const char *path, int size) {
  ...
  strncpy(newEntry.name, (part2 != NULL) ? part2 : part1, MAX_NAME - 1);
  ...
  if (part2 == NULL) {
    ...
    int slot = findFreeSubDirSlot(parent->subDirStart, parent->subDirBlocks);
    if (slot < 0) {
    ...
    ret = writeSubDirEntry(parent->subDirStart, parent->subDirBlocks, slot,
                           &newEntry);
  }
  ...
  if (ret == 0) {
    logWrite(INF, "File created: %s, size=%d, blocks=%d at %d", path, size,
...
int createDir(const char *path) {
  ...
  if (part1 == NULL || part2 != NULL) {
    logWrite(ERR, "createDir: only single-level dirs supported: %s", path);
    pthread_mutex_unlock(&globalLock);
    return -1;
  }
...
int deleteFile(const char *path) {
  ...
  if (entry->openCount > 0) {
    logWrite(ERR, "deleteFile: '%s' is currently open by %d users", path,
             entry->openCount);
    pthread_mutex_unlock(&globalLock);
    return -1;
  }
  ...
  if (part2 == NULL) {
    ...
    if (parent != NULL) {
      ...
      DirEntry e;
      for (int i = 0; i < maxEntries; i++) {
        if (readSubDirEntry(parent->subDirStart, parent->subDirBlocks, i, &e) !=
            0) {
          continue;
        }
        if (e.name[0] != '\0' && strcmp(e.name, part2) == 0) {
          writeSubDirEntry(parent->subDirStart, parent->subDirBlocks, i,
                           &emptyEntry);
          break;
        }
      }
...
int deleteDir(const char *path) {
  ...
  DirEntry e;
  for (int i = 0; i < maxEntries; i++) {
    if (readSubDirEntry(entry->subDirStart, entry->subDirBlocks, i, &e) != 0) {
      continue;
    }
    if (e.name[0] != '\0') {
  ...
  if (part1 != NULL) {
    ...
    for (int i = 0; i < MAX_DIR_ENTRIES; i++) {
      ...
      if (rootEntry.name[0] != '\0' && strcmp(rootEntry.name, part1) == 0) {
        writeRootEntry(i, &emptyEntry);
        break;
      }
    }
...
int listDir(const char *path) {
  ...
  if (strcmp(path, "/") == 0) {
    ...
    DirEntry entry;
    for (int i = 0; i < maxEntries; i++) {
      if (readSubDirEntry(parent->subDirStart, parent->subDirBlocks, i,
                          &entry) != 0) {
        continue;
      }
      if (entry.name[0] != '\0') {
...
int readFile(int fd, char *buf, int size) {
  ...
  while (bytesRead < bytesToRead) {
    ...
    memcpy(buf + bytesRead, bufferPool[slot].data + blockOffset, chunk);
    bytesRead += chunk;
  }
...
int writeFile(int fd, const char *buf, int size) {
  ...
  if (offset >= fileSize) {
    logWrite(ERR, "writeFile: offset %d beyond file size %d", offset, fileSize);
    pthread_mutex_unlock(&globalLock);
    return -1;
  }
  ...
  while (bytesWritten < bytesToWrite) {
    ...
    memcpy(bufferPool[slot].data + blockOffset, buf + bytesWritten, chunk);
    markDirty(slot);
    bytesWritten += chunk;
  }
...
void fsShutdown(void) {
  ...
  for (int i = 0; i < MAX_OPEN_FILES; i++) {
    if (openFileTable[i].used) {
      logWrite(WRN, "Force closing fd=%d (%s) on shutdown", i,
               openFileTable[i].path);
      openFileTable[i].used = 0;
    }
  }
